...
export async function POST(request: Request) {
  try {
    ...
    const result = await sql`
      SELECT id, email, full_name as name, password_hash, balance FROM buyers WHERE email = ${email}
    `
    ...
    const response = NextResponse.json(
      { success: true, buyer: { id: buyer.id, email: buyer.email, name: buyer.name, balance: buyer.balance || 0 } },
      { status: 200 }
    )
