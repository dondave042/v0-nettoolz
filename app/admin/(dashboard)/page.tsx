...
import { Package, FolderOpen, ShoppingCart, Megaphone, Archive, CreditCard, TrendingUp, DollarSign, Truck } from "lucide-react"
...
export default async function AdminDashboardPage() {
  ...
  const [pendingDeliveries] = await sql`SELECT COUNT(*)::int as count FROM orders WHERE payment_status = 'completed' AND is_delivered = false`
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-amber-900 dark:text-amber-100">
                {pendingDeliveries.count} Product{pendingDeliveries.count !== 1 ? 's' : ''} Awaiting Delivery
              </h3>
              <p className="text-sm text-amber-800 dark:text-amber-200 mt-1">
                You have completed payments that need product delivery
              </p>
            </div>
