...
function CheckoutContent() {
  ...
  const [form, setForm] = useState({
    payment_method_id: "",
    use_user_method: false, // Track if this is a user payment method
  })
  ...
  async function fetchPaymentMethods() {
    try {
      // First try to fetch user's saved payment methods
      const userRes = await fetch('/api/buyers/payment-methods')
      if (userRes.ok) {
        const userData = await userRes.json()
        if (userData.methods && userData.methods.length > 0) {
          setPaymentMethods(userData.methods)
          if (userData.methods.length > 0) {
            setForm((prev) => ({
              ...prev,
              payment_method_id: userData.methods[0].id.toString(),
              use_user_method: true,
            }))
          }
          return
        }
      ...
      // Fall back to global payment methods if no user methods
    ...
    try {
      ...
      const checkoutRes = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          ...(form.use_user_method
            ? { user_payment_method_id: parseInt(form.payment_method_id) }
